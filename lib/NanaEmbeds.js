...
class NanaEmbeds {
  ...
  async getEmoji(id, m, msg) {
    ...
    forwards.on("collect", async f => {
      ...
      */
      ...
      read.setFooter(`Page ${pagination} of ${doujin.length} / ${res.id} | Readers ${nick}`);
  ...
  async getRead(res, read, r, msg, pagination) {
    ...
    backwardsTen.on("collect", (reaction, user) => {
      r.reactions.resolve(reaction).users.remove(user);
      if (pagination <= 5) return;
      pagination -= 5;
      read.setImage(images[pagination - 1]);
      read.setFooter(`Page ${pagination} of ${images.length} / ${res.id} | Readers ${user.username}`);
      r.edit(read);
    });
    ...
    forwardsTen.on("collect", (reaction, user) => {
      r.reactions.resolve(reaction).users.remove(user);
