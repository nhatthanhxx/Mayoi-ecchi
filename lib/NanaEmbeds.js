...
class NanaEmbeds {
  ...
  async getInfoEmbed(id, msg) {
    ...
    embed.setFooter(`React with 📖 to continue reading / ${res.id} | Readers ${msg.author.username}`);
  ...
  async getEmoji(id, m, msg) {
    ...
    forwards.on("collect", async f => {
      ...
      m.delete();
      */
      // m.reactions.resolve(reaction).users.remove(this.client.user);
      // await m.react("♻");
      ...
      read.setFooter(`Page ${pagination} of ${doujin.length} / ${res.id} | Owner ${msg.author.username}`);
      let r = await msg.channel.send(read);
      return this.getRead(res, read, r, msg, pagination, user);
    });
    ...
    favorit.on("collect", (user) => {
      let favorite = await this.client.favorite.getUserFavoritID(user.id);
      favorite = favorite.map(x => x.bookID);
      if (favorite.includes(res.id)) {
        this.client.favorite.deleteUserFavoritID(user.id, res.id);
        return msg.channel
          .send(
            `${user.name} your favorite of **${res.title.pretty}** has been deleted from **Favorite**`
          )
          .then(msg => msg.delete({ timeout: 5000 }));
      } else {
        this.client.favorite.setUserFavoritID(user.id, res.id);
        return msg.channel
          .send(
            `${user.name} you have added **${res.title.pretty}** to **Favorite**`
          )
          .then(msg => msg.delete({ timeout: 5000 }));
      }
    });
  ...
  async getRead(res, read, r, msg, pagination, user) {
    let images = [];
    res.images.pages.forEach((page, i) => {
      images.push(
        `https://i.nhentai.net/galleries/${res.media_id}/${i + 1}.${
    ...
    read.setFooter(`Page ${pagination} of ${images.length} / ${res.id} | Owner ${user.username}`);
    ...
    const backwardsTenFilter = (reaction, reader) =>
      reaction.emoji.name === `⏪` && reader.id != this.client.user.id;
    const backwardsFilter = (reaction, reader) =>
      reaction.emoji.name === `⬅` && reader.id != this.client.user.id;
    const deleteFilter = (reaction, reader) =>
      reaction.emoji.name === `♻` && reader.id == msg.author.id;
    const forwardsFilter = (reaction, reader) =>
      reaction.emoji.name === `➡` && reader.id != this.client.user.id;
    const forwardsTenFilter = (reaction, reader) =>
      reaction.emoji.name === `⏩` && reader.id != this.client.user.id;
    ...
    backwardsTen.on("collect", (reaction, reader) => {
      r.reactions.resolve(reaction).reader.remove(reader);
      ...
      read.setFooter(`Page ${pagination} of ${images.length} / ${res.id} | Owner ${user.username} | Reader ${reader.id}`);
      r.edit(read);
    });

    backwards.on("collect", (reaction, reader) => {
      r.reactions.resolve(reaction).reader.remove(reader);
    ...
    forwards.on("collect", (reaction, reader) => {
      r.reactions.resolve(reaction).reader.remove(reader);
    ...
    forwardsTen.on("collect", (reaction, reader) => {
      r.reactions.resolve(reaction).reader.remove(reader);
    ...
    deletes.on("collect", (reader) => {
      if (reader.id == user.id) r.delete();
    });
  }
