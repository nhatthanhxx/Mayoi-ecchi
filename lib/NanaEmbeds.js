...
class NanaEmbeds {
  ...
  async getEmoji(id, m, msg) {
    ...
    forwards.on("collect", async f => {
      ...
      read.setFooter(`Page ${pagination} of ${doujin.length} / ${res.id} | Readers ${msg.author.username}`);
  ...
  async getRead(res, read, r, msg, pagination) {
    ...
    backwardsTen.on("collect", (reaction, user) => {
      ...
      read.setFooter(`Page ${pagination} of ${images.length} / ${res.id} | Readers ${msg.author.username}`);
      r.edit(read);
    });
